...
class Tau_initilizer(tf.keras.initializers.Initializer):
    def __init__(self, minval=0.0, maxval=2.0):
        self.minval = minval
        self.maxval = maxval

    def __call__(self, shape, dtype=None):
        return tf.constant(
            np.random.uniform(self.minval, self.maxval, size=shape),
            dtype=dtype
        )

@keras_export('keras.layers.SimpleCTRNNCell')
class SimpleCTRNNCell(DropoutRNNCellMixin, base_layer.BaseRandomLayer):
    """Cell class for SimpleCTRNN.
  ...
  """
    ...
    def build(self, input_shape):
        ...
        self.tau = self.add_weight(shape=(self.units,),
                                   initializer=Tau_initilizer, #tf.keras.initializers.Ones(),
    ...
    def call_backup(self, inputs, states, training=None):
        ...
        return output, new_state
    def call(self, inputs, states, training=None):
        #tf.print("Jalila_ctrnn")
        prev_output = states[0] if tf.nest.is_nested(states) else states
        sequence_length=int(self.input_frequency/self.h)
        ...
        for t in tf.range(sequence_length):
            ...
            h = backend.bias_add(h, self.bias)
            output = h + backend.dot(prev_output, self.recurrent_kernel)
            output=tf.math.subtract(output,prev_output)
            output=tf.math.multiply(tf.math.divide(self.h,self.tau),output)
            output=tf.math.add(output,prev_output)
        ...
        new_state = [output] if tf.nest.is_nested(states) else output
        return output, new_state
    def call_modified_euler(self, inputs, states, training=None):
        #tf.print("Nada")
        #modified euler
        prev_output = states[0] if tf.nest.is_nested(states) else states
        sequence_length=int(self.input_frequency/self.h)
        ...
        for t in range(sequence_length):
            ...
            output = h + backend.dot(prev_output, self.recurrent_kernel)
            output=tf.math.subtract(output,prev_output)
            k1=tf.math.multiply(tf.math.divide(self.h,self.tau),output)
            prediction_output = self.activation(output)

            k2 = prediction_output+h + backend.dot(prediction_output, self.recurrent_kernel)

            k2=tf.math.multiply(tf.math.divide(self.h,self.tau),k2)
            output=tf.math.multiply(tf.constant(.05),tf.math.add(k1,k2))
            output=tf.math.add(output,prev_output)
    ...
    def callode(self, inputs, states, training=None):
        import tensorflow_probability as tfp

        prev_state = states[0] if tf.nest.is_nested(states) else states

        def ode_fn(_, initial_state):
            #tf.print(initial_state.shape)
            #one_over_tau=tf.math.divide(tf.constant(1.0) , self.tau)
            # I=backend.dot(inputs, self.kernel)
            # if self.bias is not None:
            #     I = backend.bias_add(I, self.bias)
            recurrent_input=backend.dot(initial_state, self.recurrent_kernel)
            minuse_x=tf.math.multiply(tf.constant(-1.0),initial_state)
            # output=tf.math.add(minuse_x,I)
            #output=tf.math.add(output,recurrent_input)
            output = tf.math.add(minuse_x, recurrent_input)
            if self.activation is not None:
                output = self.activation(output)
            return output
        ...
        states = ode_solver.solve(ode_fn, initial_time=0.0, initial_state=prev_state, solution_times=[1.0])
        tf.print(states)
        output = states.states[-1]
    ...
    def get_config(self):
        config = {
            ...
            'bias_constraint':
                constraints.serialize(self.bias_constraint),
            'tau_constraint':
                constraints.serialize(self.tau_constraint),
            'dropout':
                self.dropout,
            'recurrent_dropout':
                self.recurrent_dropout
...
class SimpleCTRNN(RNN):
  ...
  ```python
  inputs = np.random.random([32, 10, 8]).astype(np.float32)
  simple_rnn = tf.keras.layers.SimpleCTRNN(4)

  output = simple_rnn(inputs)  # The output has shape `[32, 4]`.

  simple_rnn = tf.keras.layers.SimpleCTRNN(
      4, return_sequences=True, return_state=True)
...
class ShuttleCTRNNCell(DropoutRNNCellMixin, base_layer.BaseRandomLayer):
  ...
  """
    ...
    def call_bu(self, inputs, states, training=None):
        ...
        next_z=tf.math.multiply(next_z,self.output_gain)
        #tf.print(next_z)
...
##############Real Shuttle Design##################
##############################################
@keras_export('keras.layers.RealShuttleCTRNNCell')
class RealShuttleCTRNNCell(DropoutRNNCellMixin, base_layer.BaseRandomLayer):
    """Cell class for ShuttleCTRNN.
  ...
  ```python
  inputs = np.random.random([32, 10, 8]).astype(np.float32)
  rnn = tf.keras.layers.RNN(tf.keras.layers.RealShuttleCTRNNCell(4))

  output = rnn(inputs)  # The output has shape `[32, 4]`.

  rnn = tf.keras.layers.RNN(
      tf.keras.layers.RealShuttleCTRNNCell(4),
      return_sequences=True,
      return_state=True)
  ...
  """
    ...
    def __init__(self,
                 ...
                 dropout=0.,
                 recurrent_dropout=0.,
                 omega_0=2*(22/7) * 3727.58,
                 input_gain=10,
                 output_gain=(10**15)*10*(10**-3),
                 zeta=1.61,
                 vb=10.0,
                 mass=14.1 * (10 ** -9),
                 d=4.5 * (10 ** -6),  # gap
                 eps=8.854 * (10 ** -12),
                 A=58.7 * (10 ** -9),  # Excitation Area
                 Am=100 * (10 ** -9),  # tunning area
                 dsUp=4.5* (10**-6),
                 dsDown=13.5* (10**-6),
                 Area_sensing=69.1 * (10**-9),
                 deltaT=10**-6,
                 K_B=1.380649*10**-23,
                 T=25+273.15,
                 **kwargs):
        ...
        if tf.compat.v1.executing_eagerly_outside_functions():
            ...
            self._enable_caching_device = kwargs.pop('enable_caching_device', False)
        super(RealShuttleCTRNNCell, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
        self.use_bias = use_bias
        ...
        self.K_B=K_B,
        self.T=T,
        self.Area_sensing = Area_sensing,
        self.half_eps_mul_A=0.5*self.eps*self.A,
        self.half_eps_mul_Am=0.5*self.eps*self.Am,
        self.eps_mul_Area_sensing=eps*Area_sensing,
        ...
        self.eps_A_over_d_cubic =(self.eps* self.A)/(self.d**3),
        self.deltaT=deltaT,
        self.noral_dist = tfd.Normal(loc=0., scale=1)
    ...
    def call(self, inputs, states, training=None):
        ...
        vb_plus_vi= tf.math.add(tf.cast(self.vb, tf.float32),h)
        d_minus_z=tf.math.subtract(self.d,prev_output)
        vb_minus_vi = tf.math.subtract(tf.cast(self.vb, tf.float32), h)
        d_plus_z = tf.math.add(self.d, prev_output)
        dsUp_plus_Z=tf.math.add(self.dsUp,prev_output)
        dsUp_minus_Z=tf.math.subtract(self.dsUp,prev_output)
        dsDown_plus_Z = tf.math.add(self.dsDown, prev_output)
        dsDown_minus_Z = tf.math.subtract(self.dsDown, prev_output)
        temporary = tf.math.subtract(tf.math.pow(tf.math.divide(vb_plus_vi, d_minus_z), 2),
                                     tf.math.pow(tf.math.divide(vb_minus_vi, d_plus_z), 2))
        force_Excitation=tf.math.multiply(self.half_eps_mul_A,temporary)
        temporary1=tf.math.subtract(tf.math.pow(tf.math.divide(self.recurrent_kernel, d_minus_z), 2),
                                     tf.math.pow(tf.math.divide(self.recurrent_kernel, d_plus_z), 2))
        force_tuning=tf.math.multiply(self.half_eps_mul_Am,temporary1)

        # tmp = tf.math.multiply(4.0 * 2.0, self.K_B)
        # tmp = tf.math.multiply(tmp, self.T)
        # tmp = tf.math.multiply(tmp, self.zeta)
        # tmp = tf.math.multiply(tmp, self.omega_0)
        #force_thermo_mechnical_noise = tf.math.pow(tmp, 0.5)
        #force_thermo_mechnical_noise_divide_by_mass = tf.math.divide(force_thermo_mechnical_noise, self.mass)


        # Define a single scalar Normal distribution.
        #sample=self.noral_dist.sample([1])
        #random_float = tf.random.uniform(shape=(), minval=1.2, maxval=1.4)
        #random_float=1.0
        #tf.print(random_float)
        #tf.print(sample)
        #noise_force=tf.math.multiply(sample,force_thermo_mechnical_noise)

        #tf.print(force_thermo_mechnical_noise_divide_by_mass)
        #force_thermo_mechnical_noise_divide_by_mass=tf.math.divide(force_thermo_mechnical_noise)
        force = tf.math.add(force_Excitation, force_tuning)
        #tf.print(noise_force)

        #tf.print(force)
        #force=tf.math.add(force,noise_force)
        #self.two_zeta_omega_note_m = (2 * self.zeta * self.omega_0 *random_float* self.mass),
        Z_dot_first_term = tf.math.divide(force, self.two_zeta_omega_note_m)
        Z_dot_second_term = tf.math.multiply(self.omega_0_divided_by_two_zeta, prev_output)
        z_dot = tf.math.subtract(Z_dot_first_term, Z_dot_second_term)
        next_z = tf.math.add(prev_output, tf.math.multiply(self.deltaT, z_dot))
    ...
    #Abdallah
        capSensePositive=tf.math.add(tf.math.divide(1, dsUp_plus_Z),(tf.math.divide(1, dsDown_minus_Z)) )
        capSenseNegative=tf.math.add(tf.math.divide(1, dsDown_plus_Z),(tf.math.divide(1, dsUp_minus_Z)) )
        differenceCapSens=tf.math.subtract(capSensePositive, capSenseNegative)
        CapSen=tf.math.multiply(self.eps_mul_Area_sensing, differenceCapSens)
        output = tf.math.multiply(CapSen,self.output_gain)
        new_state = [next_z] if tf.nest.is_nested(states) else next_z
        return output, new_state  # send gain and without gain.
    ...
    def get_config(self):
        ...
        config.update(_config_for_enable_caching_device(self))
        base_config = super(RealShuttleCTRNNCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


@keras_export('keras.layers.RealShuttleCTRNN')
class RealShuttleCTRNN(RNN):
    """Fully-connected RNN where the output is to be fed back to input.
  ...
  ```python
  inputs = np.random.random([32, 10, 8]).astype(np.float32)
  simple_rnn = tf.keras.layers.RealShuttleCTRNN(4)

  output = simple_rnn(inputs)  # The output has shape `[32, 4]`.

  simple_rnn = tf.keras.layers.RealShuttleCTRNN(
      4, return_sequences=True, return_state=True)
  ...
  """
    ...
    def __init__(self,
        ...
        if 'implementation' in kwargs:
            kwargs.pop('implementation')
            logging.warning('The `implementation` argument '
                            'in `RealShuttleCTRNN` has been deprecated. '
                            'Please remove it from your layer call.')
        if 'enable_caching_device' in kwargs:
            cell_kwargs = {'enable_caching_device':
                               kwargs.pop('enable_caching_device')}
        ...
        cell = RealShuttleCTRNNCell(
            ...
            deltaT=deltaT,
            **cell_kwargs)
        super(RealShuttleCTRNN, self).__init__(
            cell,
            return_sequences=return_sequences,
            return_state=return_state,
            go_backwards=go_backwards,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        print("RNN")
        return super(RealShuttleCTRNN, self).call(
            inputs, mask=mask, training=training, initial_state=initial_state)
    ...
    def get_config(self):
        ...
        base_config = super(RealShuttleCTRNN, self).get_config()
        config.update(_config_for_enable_caching_device(self.cell))
        del base_config['cell']
        return dict(list(base_config.items()) + list(config.items()))
