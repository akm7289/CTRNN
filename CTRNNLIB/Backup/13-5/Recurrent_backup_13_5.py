...
class SimpleCTRNNCell(DropoutRNNCellMixin, base_layer.BaseRandomLayer):
  ...
  def __init__(self,
    ...
    self.state_size = self.units
    self.output_size = self.units
    self.tau_reciprocal = tau_reciprocal
  ...
  def call(self, inputs, states, training=None):
    print("SimpleCTRNNCALL ",str(self.tau_reciprocal))
    ...
    if self.activation is not None:
      output = self.activation(output)
    output=tf.constant(1-self.tau_reciprocal)*prev_output+ tf.constant(self.tau_reciprocal)*output
    new_state = [output] if tf.nest.is_nested(states) else output
    return output, new_state
...
def updadte_voltage(inputs,states):
  omg=1209.5*20
  zeta = 1.028075
  tau = (2 * zeta) / omg
  dt = 0.0001
...
class SimpleMEMSCTRNNCell(DropoutRNNCellMixin, base_layer.BaseRandomLayer):
  ...
  def call(self, inputs, states, training=None):
    ...
    z_mul_93= tf.math.multiply(z , 0.93)
    a= tf.constant(4.9311e-20) # define as hyper-paramter in the future
    d= tf.constant(41.8e-6)# define as hyper-paramter in the future
    ...
    if self.implementation == 1:
      ...
      new_z=tf.math.add(z_mul_93,term2)
      ...
      new_z_minus_d=tf.math.subtract(new_z,d)
      #sigmoid_input=tf.math.multiply(new_z, 50000000)
      #sigmiod__new_z_minus_d=tf.experimental.numpy.heaviside(new_z_minus_d ,d)
      sigmiod__new_z_minus_d=tf.keras.activations.sigmoid(tf.math.multiply(new_z_minus_d, 50000)) #5 after five zeros it stop learnig
      #sigmiod__new_z_minus_d = tf.keras.activations.tanh(new_z - 41.8 * (10 ** -6))
